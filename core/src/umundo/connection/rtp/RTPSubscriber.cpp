...
void RTPSubscriber::init(const Options* config) {
	...
	if (config->getKVPs().count("pub.rtp.multicast") &&
			!config->getKVPs().count("pub.rtp.portbase")) {
	...
	if(config->getKVPs().count("sub.rtp.multicast")) {
		...
		status = libre::sa_set_str(&maddr, _multicastIP.c_str(), _port);
		
		if(status)
			UM_LOG_WARN("%s: error in libre::sa_set_str(%s:%u): %s", SHORT_UUID(_uuid).c_str(), _multicastIP.c_str(), _port, strerror(status));
