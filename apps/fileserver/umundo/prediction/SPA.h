...
public:
	...
	list<list<Occurences> > getPriorOccurences() {
		return getPriorOccurences(_historyWindow);
	};
	list<string> historyToKey(int count, ... );
	list<string> historyToKey(const list<string>& history);
	list<string> historyToKey() {
		return historyToKey(_historyWindow);
	}
...
public:
	...
	virtual map<string, float> getPredictions() {
		return getPredictions(_model->_historyWindow);
	};
	vector<std::pair<string, float> > getSortedPredictions(list<string> history);
	vector<std::pair<string, float> > getSortedPredictions() {
		return getSortedPredictions(_model->_historyWindow);
	}
